...
def test_font_info_fallback_in_instance(ufo_module, data_dir, caplog):
    """Test that font info attributes are correctly copied according to the designspace docs fallback
    rules [1]. Notes: language "de" is 0x0407/1031.
    ...
    def _validate_name_record_in_list(dict, name_records):
        return any(
            x["platformID"] == dict["platformID"]
            and x["encodingID"] == dict["encodingID"]
            and x["languageID"] == dict["languageID"]
            and x["nameID"] == dict["nameID"]
            and x["string"] == dict["string"]
    ...
    assert _validate_name_record_in_list({
        "platformID": 3,
        "encodingID": 1,
        "languageID": 1031,
        "nameID": 0,
    ...
    assert caplog.records[0].message.startswith("The given instance")
    ...
    assert _validate_name_record_in_list({
        ...
        "nameID": 0,
        "string": "Urheberrecht 2025",
    }, instance_regular_font.info.openTypeNameRecords) is False
